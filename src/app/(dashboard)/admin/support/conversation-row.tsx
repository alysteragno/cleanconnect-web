...
// A single row in the support conversation list. Owns the archive/restore
// control so it can animate itself out (collapse + fade) the moment the
// action succeeds, instead of waiting for a full page refresh to make it
// disappear abruptly. On touch devices it also supports swiping the row left
// to reveal the same action (there's no hover to rely on there); on
// mouse/desktop the tap-to-reveal icon button still works as before.
  ...
  return (
    <div className="relative overflow-hidden">
      ...
      <div
        ...
        className={`group relative flex items-center gap-3 px-4 py-3.5 bg-white hover:bg-gray-50 transition-colors ${
          exiting ? 'animate-row-collapse pointer-events-none' : ''
        }`}
      >
        <Link href={`${basePath}/support/${customerId}`} aria-label={customerName} className="absolute inset-0" />

        <div className="press-row flex items-center gap-3 flex-1 min-w-0 pointer-events-none">
          <div className={`w-9 h-9 rounded-full ${avatarColorClass} text-white flex items-center justify-center text-xs font-bold shrink-0 select-none`}>
            {initials}
          </div>
          ...
          <div className="text-right shrink-0 ml-2 flex flex-col items-end gap-1">
            ...
            {!archived && daysUntilArchive !== null && (
              <p className={`text-[10px] whitespace-nowrap ${daysUntilArchive <= 3 ? 'text-amber-500' : 'text-gray-300'}`}>
                Archives in {daysUntilArchive <= 1 ? '1 day' : `${daysUntilArchive} days`}
              </p>
            )}
            <svg
        ...
        >
          ...
          >
            ...
            <p className="text-xs text-gray-500 mt-1.5 leading-relaxed">
              Your chat with <span className="font-medium text-gray-700">{customerName}</span> will
              move out of the active list to keep things tidy. No data is deleted — the full
              conversation is preserved and you can restore it anytime from the
