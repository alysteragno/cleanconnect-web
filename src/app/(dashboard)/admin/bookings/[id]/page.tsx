...
const SERVICE_LABELS: Record<string, string> = {
  general: 'General Home Cleaning', premium_mattress: 'Mattress & Upholstery',
...
export default async function AdminBookingDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: booking }, { data: assignments }] = await Promise.all([
    supabase.from('bookings')
      .select('id, service_date, service_time, service_type, space_type, property_sqm, required_cleaners, duration_hours, base_price, status, payment_status, payment_method, address_unit, address_street, address_city, address_province, special_notes, profiles!customer_id (full_name, phone), branches (name)')
      .eq('id', id).single(),
    supabase.from('cleaner_assignments')
      .select('status, profiles!cleaner_id (full_name)')
      .eq('booking_id', id),
  ])

  if (!booking) notFound()
  const b = booking as unknown as Booking
  const aList = (assignments ?? []) as unknown as Assignment[]
  const address = [b.address_unit, b.address_street, b.address_city, b.address_province].filter(Boolean).join(', ')

  return (
    <div className="max-w-4xl">
      <div className="mb-6">
        <Link href="/admin/bookings" className="text-sm text-gray-400 hover:text-gray-600 transition-colors">← All Bookings</Link>
        ...
        <p className="text-sm text-gray-500 mt-0.5">{b.branches?.name}</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-5">
          ...
          {aList.length > 0 && (
            <div className="bg-white rounded-xl border border-gray-200 p-5 space-y-3">
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide">Cleaner Assignments</p>
              <div className="space-y-2">
                {aList.map((a, i) => (
                  <div key={i} className="flex items-center justify-between px-3 py-2 border border-gray-100 rounded-lg">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-full border font-medium capitalize ${ASSIGN_BADGE[a.status] ?? ''}`}>
                      {a.status}
                    </span>
                  </div>
                ))}
